...
from .utils import trimite_mail_admin_custom
...
def task_notificare_profil():
    nzl = getattr(settings, 'NZL_ZILE_NOTIFICARE', 3)
    acm = timezone.now()
    ...
    for user in users:
        ...
        if lipsa:
            if not user.last_notification_date or (acm - user.last_notification_date).days >= nzl:
                user.needs_profile_notification = True
                user.last_notification_date = acm
                user.save()
                logger.info(f"TASK PROFILE: Utilizator {user.username} marcat pentru notificare.")
